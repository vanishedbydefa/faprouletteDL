...
def get_max_id_from_db(db_path:str):
    ...
    if highest_id_downloaded == None and highest_id_general == None:
        ...
        logger.error("It seems like one table in the database is missing.\n",
                    "This should not occure from normal usage of 'faproulette-downloader'.\n",
                    "The program itself will not fix this issue. Think about deleting your DB and start downloading from zero again\n",
                    "You can also download an existing DB from github and use the downloader in speed mode.")
    elif highest_id_downloaded < highest_id_general:
        logger.info(f"highest ID: {highest_id_general}")
        return highest_id_general
    else:
        logger.info(f"highest ID: {highest_id_downloaded}")
...
def check_db_exists(db_path:str):
